...
pub fn to_kana_with_opt(orig: &str, options: Options) -> String {
    ...
    while curr_pos != len  {
        ...
        if result.1 == 0{
            ...
            // dbg!(result.1);
            if chars[curr_pos.. curr_pos + result.1].iter().all(|c|char::is_uppercase(*c)){
...
impl Node {
    fn get<'a>(&self, chars: &'a [char]) -> (Option<&'static str>, usize) {
        let mut i = 0;
...
pub static ref TO_KANA_NODE_TREE: Node = {
...
pub static ref TO_KANA_NODE_TREE_OBSOLETE: Node = {
    let mut tree = TO_KANA_NODE_TREE.clone();
    let w = tree.find_transition_mut('w').unwrap();
    w.1.transitions = w.1.transitions.iter().filter(|x| x.0 != 'i' && x.0 != 'e').cloned().collect();
    w.1.transitions.push(('i', Node{transitions: vec![], output: Some("ゐ")}));
    w.1.transitions.push(('e', Node{transitions: vec![], output: Some("ゑ")}));
    tree
};

pub static ref TO_KANA_NODE_TREE_IMEMODE: Node = {
    let mut tree = TO_KANA_NODE_TREE.clone();
    let w = tree.find_transition_mut('n').unwrap();
    w.1.transitions.push(('n', Node{transitions: vec![], output: Some("ん")}));
    w.1.transitions.push((' ', Node{transitions: vec![], output: Some("ん")}));
    w.1.output = None;
    tree
};
